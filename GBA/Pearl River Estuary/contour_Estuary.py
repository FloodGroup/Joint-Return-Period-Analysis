"""
Compound flood return-period analysis (GBA Estuary — mainland).

Estimates bivariate rainfall–tide return periods with copula-based joint distributions.
For Hong Kong stations, see HongKong/contour_HongKong.py.
...
# New xlsx data loading functions
def load_water_level_xlsx(file_path):
    """Internal helper."""
    import pandas as pd
    ...
    try:
        ...
        while i < len(columns) - 1:
            col1 = str(columns[i]).strip()
            col2 = str(columns[i + 1]).strip()
            
                year_data_pairs.append((i, i + 1, col1, col2))
                if VERBOSE:
                    # logging omitted
                i += 2
            else:
        ...
        if not year_data_pairs:
            # logging omitted
            return None
            
        all_data = []
        
        for year_col_idx, data_col_idx, year_col_name, data_col_name in year_data_pairs:
            if VERBOSE:
                # logging omitted
            
            year_data = df.iloc[:, [year_col_idx, data_col_idx]].copy()
            year_data.columns = ['Date', 'Value']
            
            year_data = year_data.dropna(subset=['Date', 'Value'])
            ...
            for idx, row in year_data.iterrows():
                try:
                    ...
                    if pd.notna(date_obj) and not pd.isna(value):
            ...
            for date_obj, value in zip(valid_dates, valid_values):
                all_data.append({'DATE': date_obj, 'Sea_Level': value})
...
def load_rainfall_xlsx(file_path):
    """Internal helper."""
    import pandas as pd
    from datetime import datetime, timedelta
    ...
    try:
        ...
        for year_col_idx, data_col_idx, year_col_name, data_col_name in year_data_pairs:
            ...
            year_match = None
            for char in year_col_name:
                if char.isdigit():
                    year_str = ''.join([c for c in year_col_name if c.isdigit()])
                    if len(year_str) == 4:
            ...
            start_date = datetime(year_match, 1, 1)
            
            is_leap = (year_match % 4 == 0 and year_match % 100 != 0) or (year_match % 400 == 0)
            ...
            for day_offset in range(days_in_year):
                ...
                if current_date in date_value_map:
                    rainfall_value = date_value_map[current_date]
                else:
                    rainfall_value = 0.0
                    
                all_data.append({'DATE': current_date, 'Rainfall': rainfall_value})
...
def load_data_xlsx(water_level_path, rainfall_path, offset_file_path=None):
    """Internal helper."""
    if VERBOSE:
        # logging omitted
    ...
    if VERBOSE:
        # logging omitted
    tide_data = load_water_level_xlsx(water_level_path)
    ...
    if VERBOSE:
        # logging omitted
    rainfall_data = load_rainfall_xlsx(rainfall_path)
    ...
    if offset_file_path:
        ...
        if offset_data is not None:
            tide_data = apply_datum_correction(tide_data, offset_data, 'Sea_Level')
            if VERBOSE:
                # logging omitted
        else:
            if VERBOSE:
    ...
    if 'Sea_Level' in tide_data.columns:
        if 'Sea_Level_Zhujiang' not in tide_data.columns:
            tide_data['Sea_Level_Zhujiang'] = tide_data['Sea_Level']
        tide_data['Sea_Level'] = tide_data['Sea_Level'] + 0.744
        tide_data['Applied_Reference_Shift'] = 0.744
        tide_data['Reference'] = '1985_CN_Height_Datum'
        try:
            min_d = tide_data['DATE'].min().date()
            max_d = tide_data['DATE'].max().date()
        except Exception:
            min_d = tide_data['DATE'].min()
            max_d = tide_data['DATE'].max()
        if VERBOSE:
            # logging omitted
            # logging omitted
            # logging omitted
    
    if 'Year' not in tide_data.columns:
        tide_data['Year'] = tide_data['DATE'].dt.year
    if 'Month' not in tide_data.columns:
        tide_data['Month'] = tide_data['DATE'].dt.month
    if 'Day' not in tide_data.columns:
        tide_data['Day'] = tide_data['DATE'].dt.day
        
    if 'Year' not in rainfall_data.columns:
        rainfall_data['Year'] = rainfall_data['DATE'].dt.year
    if 'Month' not in rainfall_data.columns:
        rainfall_data['Month'] = rainfall_data['DATE'].dt.month
    if 'Day' not in rainfall_data.columns:
        rainfall_data['Day'] = rainfall_data['DATE'].dt.day
    
    
    return tide_data, rainfall_data

# Original CSV load data function (kept for compatibility)
...
def create_compound_events(tide_data, rainfall_data, rainy_day_def=0.1, rainfall_threshold_method='rainy_days', threshold_percentile=95):
    """
    ...
    print(f"\nCalculating initial {threshold_percentile}th percentile thresholds for event identification...")
    ...
    if len(valid_sea_level) < 20 or len(valid_rainfall) < 20: # Check if enough data for percentiles
        print(f"Warning: Insufficient data points to reliably calculate {threshold_percentile}th percentiles.")
        sea_level_95th = np.nan
        rainfall_95th_overall = np.nan
        rainfall_95th_rainy = np.nan
    else:
        sea_level_95th = np.percentile(valid_sea_level, threshold_percentile)
        print(f"  Sea level {threshold_percentile}th percentile (overall): {sea_level_95th:.3f} m")

        # Rainfall threshold based on method
        rainfall_95th_overall = np.percentile(valid_rainfall, threshold_percentile)
        print(f"  Rainfall {threshold_percentile}th percentile (all days): {rainfall_95th_overall:.2f} mm")

        rainy_days_data = valid_rainfall[valid_rainfall > rainy_day_def]
        if len(rainy_days_data) >= 20: # Check enough rainy days
            rainfall_95th_rainy = np.percentile(rainy_days_data, threshold_percentile)
            print(f"  Rainfall {threshold_percentile}th percentile (rainy days > {rainy_day_def} mm): {rainfall_95th_rainy:.2f} mm")
        else:
            print(f"  Warning: Insufficient rainy days ({len(rainy_days_data)}) for rainy day {threshold_percentile}th percentile. Using overall threshold.")
    ...
    if rainfall_threshold_method == 'rainy_days':
        ...
        if VERBOSE:
            print(f"Using RAINY DAY {threshold_percentile}th percentile for initial event definition: {initial_rainfall_threshold:.2f} mm")
    elif rainfall_threshold_method == 'all_days':
        initial_rainfall_threshold = rainfall_95th_overall
        if VERBOSE:
            print(f"Using ALL DAY {threshold_percentile}th percentile for initial event definition: {initial_rainfall_threshold:.2f} mm")
    else:
    ...
    if VERBOSE:
        print(f"Using Sea level {threshold_percentile}th percentile for initial event definition: {initial_sea_level_threshold:.3f} m")
    ...
    print(f"Thresholds: Rainfall={initial_rainfall_threshold:.2f} mm, Sea_Level={initial_sea_level_threshold:.3f} m")
    print(f"Samples: Compound initial={compound_extreme_count}")
...
def fit_copula_models(compound_data, rainfall_col, sea_level_col, rainfall_threshold=None, sea_level_threshold=None):
    ...
    """
    # Calculate 90th percentile if thresholds not provided
    if rainfall_threshold is None:
        rainfall_threshold = np.percentile(compound_data[rainfall_col], 90)
    if sea_level_threshold is None:
        sea_level_threshold = np.percentile(compound_data[sea_level_col], 90)
    ...
    u = np.clip(u_values.values, 1e-6, 1 - 1e-6)
    v = np.clip(v_values.values, 1e-6, 1 - 1e-6)
...
def create_multi_station_figure(station_configs, figsize=(18, 12)):
    ...
    for idx, config in enumerate(station_configs):
        ...
        try:
            ...
            tide_data, rainfall_data = load_data_xlsx(water_level_file, rainfall_file, 
            ...
            if station_name == 'SSW':
                station_percentile = 85
            elif station_name in ['NS', 'DS']:
                station_percentile = 90
            else:
                station_percentile = 95
            compound_data, rainfall_threshold, sea_level_threshold = create_compound_events(
                tide_data, rainfall_data, 
                rainy_day_def=0.1,
                rainfall_threshold_method='rainy_days',
                threshold_percentile=station_percentile
            )
            ...
            enabled_copulas = config.get('enabled_copulas', None)
            force_copula = config.get('force_copula', None)
            copula_results, best_copula, fits = fit_copula_models_selective(
                ...
                enabled_copulas=enabled_copulas,
                force_copula=force_copula
            )
...
def fit_copula_models_selective(compound_data, rainfall_col, sea_level_col, rainfall_threshold=None, sea_level_threshold=None, enabled_distributions=None, enabled_copulas=None, force_copula=None):
    """Internal helper."""
    # Calculate 90th percentile if thresholds not provided
    if rainfall_threshold is None:
        rainfall_threshold = np.percentile(compound_data[rainfall_col], 90)
    ...
    for col, fit in fits.items():
        ...
        if isinstance(params, (list, tuple, np.ndarray)):
            param_str = ', '.join([f"{p:.4f}" if isinstance(p, (int, float, np.floating)) else str(p) for p in (list(params)[:3])])
        else:
            param_str = str(params)
        if 'threshold' in fit:
            param_str = f"{param_str}, thr={fit['threshold']:.4f}"
        print(f"Best marginal - {col}: {dist_name}, BIC={bic_val:.2f}, KS p={ks_p_val_str}, Params=({param_str})")
    ...
    results = {}
    all_copulas = {'gaussian','t','clayton','gumbel','frank','joe','survival_clayton'}
    if enabled_copulas is None:
        enabled_copulas = all_copulas
    else:
        enabled_copulas = {c.lower() for c in enabled_copulas} & all_copulas
        if not enabled_copulas:
            enabled_copulas = all_copulas

    if force_copula is not None:
        force_copula = str(force_copula).lower()
        if force_copula not in all_copulas:
            # logging omitted
            force_copula = None
    ...
    def calculate_nll_from_pdf(copula_model, data, model_name):
        ...
        log_pdf_values = np.log(pdf_values)
        if np.any(~np.isfinite(log_pdf_values)):
            return np.inf
        log_lik = np.sum(log_pdf_values)
        nll = -log_lik
        if not np.isfinite(nll):
    ...
    try:
        n = len(data)
        # Gaussian Copula
        if ('gaussian' in enabled_copulas) and (force_copula in (None, 'gaussian')):
            cop_gaussian = GaussianCopula(dim=2)
            cop_gaussian.fit(data)
            nll = calculate_nll_from_pdf(cop_gaussian, data, "Gaussian")
        ...
        # Student-t Copula
        if ('t' in enabled_copulas) and (force_copula in (None, 't')):
            cop_student = StudentCopula(dim=2)
            cop_student.fit(data)
        ...
        # Clayton Copula
        if ('clayton' in enabled_copulas) and (force_copula in (None, 'clayton')):
            cop_clayton = ClaytonCopula(dim=2)
            cop_clayton.fit(data)
            nll = calculate_nll_from_pdf(cop_clayton, data, "Clayton")
            k = 1
        ...
        # Gumbel Copula
        if ('gumbel' in enabled_copulas) and (force_copula in (None, 'gumbel')):
            cop_gumbel = GumbelCopula(dim=2)
            cop_gumbel.fit(data)
            nll = calculate_nll_from_pdf(cop_gumbel, data, "Gumbel")
            k = 1
        ...
        # Frank Copula
        if ('frank' in enabled_copulas) and (force_copula in (None, 'frank')):
            cop_frank = FrankCopula(dim=2)
            cop_frank.fit(data)
            nll = calculate_nll_from_pdf(cop_frank, data, "Frank")
            k = 1
        ...
        # Joe Copula
        if ('joe' in enabled_copulas) and (force_copula in (None, 'joe')):
            cop_joe = SimpleJoeCopula(dim=2)
            cop_joe.fit(data)
            nll = calculate_nll_from_pdf(cop_joe, data, "Joe")
            k = 1
        ...
        if ('survival_clayton' in enabled_copulas) and (force_copula in (None, 'survival_clayton')):
            u_orig = data[:, 0]
            v_orig = data[:, 1]
            u_survival_transform = 1 - u_orig
            v_survival_transform = 1 - v_orig
    ...
    if force_copula and (force_copula in results):
        best_copula_name = force_copula
        print(f"Copula selected: {best_copula_name}")
    else:
        if passed_ks_test_models:
            print(f"\nModels passing K-S test (p > {ks_significance_level}): {list(passed_ks_test_models.keys())}")
            best_copula_name = min(passed_ks_test_models.items(), key=lambda x: x[1]['bic'])[0]
...
def main_multi_station():    
    ...
    station_configs = [
        {
            'name': 'CW',
            'water_level_file': str(DATA_DIR / 'CW_water_level.xlsx'),
            'rainfall_file': str(DATA_DIR / 'TG_rainfall.xlsx'),
            'offset_file': str(DATA_DIR / 'CW_autum.csv'),
            'title': 'CW',
            'x_max': 4.5,
            'y_max': 350,
            'enabled_distributions': [
            ...
            'enabled_copulas': ['t', 'clayton', 'frank', 'joe', 'survival_clayton']
        },
        {
            'name': 'SZ',
            'water_level_file': str(DATA_DIR / 'SZ_water_level.xlsx'),
            'rainfall_file': str(DATA_DIR / 'SZ_rainfall.xlsx'),
            'offset_file': str(DATA_DIR / 'SZ_autum.csv'),
            'title': 'SZ',
            'x_max': 5.0,
            'y_max': 350,
            'enabled_distributions': [
            ...
            'enabled_copulas': ['gaussian', 't', 'clayton', 'frank', 'joe', 'survival_clayton']
        },
        {
            'name': 'NLT',
            'water_level_file': str(DATA_DIR / 'NLT_water_level.xlsx'),
            'rainfall_file': str(DATA_DIR / 'CJ_rainfall.xlsx'),
            'offset_file': str(DATA_DIR / 'NLT_autum.csv'),
            'title': 'NLT',
            'x_max': 4.0,
            'y_max': 450,
            'enabled_distributions': [
    ...
    print("Building multi-station return-period contours...")
    fig = create_multi_station_figure(station_configs, figsize=(18, 14))
    print("Multi-station analysis complete.")
    return fig
