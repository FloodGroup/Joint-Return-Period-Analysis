...
COPULA_MODELS = (
    'gaussian', 't', 'clayton', 'gumbel', 'frank', 'joe', 'survival_clayton',
)

def resolve_copula_candidates(excluded_copulas=None):
    """Return standard copula candidates, optionally omitting excluded forms."""
    if not excluded_copulas:
        return set(COPULA_MODELS)
    excluded = {c.lower() for c in excluded_copulas}
    candidates = {c for c in COPULA_MODELS if c not in excluded}
    return candidates if candidates else set(COPULA_MODELS)
    ...
    for idx, config in enumerate(station_configs):
        ...
        try:
            ...
            copula_results, best_copula, fits = fit_copula_models_selective(
                ...
                excluded_copulas=config.get('excluded_copulas'),
                force_copula=config.get('force_copula'),
            )
...
def fit_copula_models_selective(compound_data, rainfall_col, sea_level_col, rainfall_threshold=None, sea_level_threshold=None,
                               excluded_distributions=None, excluded_copulas=None, force_copula=None):
    """Internal helper."""
    ...
    enabled_copulas = resolve_copula_candidates(excluded_copulas)
    if force_copula is not None:
        force_copula = str(force_copula).lower()
        if force_copula not in COPULA_MODELS:
            force_copula = None
    ...
    station_configs = [
        {
            ...
            'x_max': 4.5,
            'y_max': 350,
            'excluded_copulas': ['gaussian', 'gumbel'],
        },
        {
            'name': 'SZ',
